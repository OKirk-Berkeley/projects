    ...
    if scheme_symbolp(signature):
        ...
        def f(formals):
            ...
            if not scheme_symbolp(formals.first):
                raise SchemeError(f'non-symbol: {formals.first}')
            return Pair(formals.first, f(formals.rest))
        formals = f(formals)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        current_pair = bindings.first
        validate_form(current_pair, 2, 2)
        if current_pair.first in taken_symbols or not scheme_symbolp(current_pair.first):
            raise SchemeError('choose a better symbol dummy')
        names = Pair(current_pair.first, names)
        vals = Pair(scheme_eval(current_pair.rest.first, env), vals)
        taken_symbols.append(current_pair.first)
