...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if type(self.exit) == Place or type(self.exit) == Water:
...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12python3 ok -q 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def helper(place, upper_bound):
            if not(place.is_hive) and upper_bound >= 0:
                if place.bees:
                    return random_bee(place.bees)
                else:
                    return helper(place.entrance, upper_bound-1)
            return None
        place_to_check = self.place
        ub = self.upper_bound
        for _ in range(self.lower_bound):
            place_to_check, ub = place_to_check.entrance, ub-1
            if place_to_check.is_hive:
                return None
        return helper(place_to_check, ub)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            bee_amount = amount + self.damage
        else:
            bee_amount = amount
        bees_copy = self.place.bees[:]
        for bee in bees_copy:
            bee.reduce_health(bee_amount)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while p.exit:
            p = p.exit
            if p.ant:
                if isinstance(p.ant, ContainerAnt) and p.ant.ant_contained:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        if target is not None:
            def helper(turns_left):
                print('DEBUG:', f'helper w/ {turns_left} turns left')
                def new_action(gamestate):
                    if turns_left <= 0 or gamestate.time % 2 == 0:
                        Bee.action(target, gamestate)
                    target.action = helper(turns_left - 1)
                return new_action
            target.action = helper(5)
        # END Problem EC 1
