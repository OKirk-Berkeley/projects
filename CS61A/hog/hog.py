...
def boar_brawl(player_score, opponent_score):
    ...
    opponent_ones = (opponent_score // 10) % 10
    brawl = 3 * abs(player_ones - opponent_ones)
    return max(1, brawl)
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    control = strategy(0, 0)
    counter1 = 0
    while (counter1 < goal):
        counter2 = 0
        while (counter2 < goal):
            if (control != strategy(counter1, counter2)):
                return False
            counter2 += 1
        counter1 += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_value = 0
    n = 1
    av = make_averaged(roll_dice, times_called)
    while (n <= 10):
        temp = av(n, dice)
        if (temp > max_value):
            max_value = temp
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if (score - opponent_score > 15):
        return sus_strategy(score, opponent_score, make_averaged(roll_dice, 100)(4), 4)
    elif (score - opponent_score < -25):
        return sus_strategy(score, opponent_score, make_averaged(roll_dice, 100)(10), 10)
    elif (score - opponent_score < -15):
        return sus_strategy(score, opponent_score, make_averaged(roll_dice, 100)(8), 8)
    return sus_strategy(score, opponent_score, make_averaged(roll_dice, 100)(6), 6)
    # END PROBLEM 12
